...
# a linear search does not need data to be sorted but a binary search does 
...
# To get to that desired number it would take 6 steps.

# how many steps would it take to get to a desired number of 150 using binary search? 
# Please write your response using complete sentences.
listB = [1,40, 44, 55, 70, 93, 99, 134, 145, 150, 200, 244]
# To get to the desried number it would take 3 steps.

# 4. In your own words describe what an algorithm is. 
# Please write your response using complete sentences.
# An algorithm is a step by step instructions of data.
...
# Please write your response using complete sentences.
# The family knows what floor there visting and the floors numbers aren't mixed up there
# in order so it would take a constant amount for them to get to floor 7. So constant time complexity. 
...
# Please write your response using complete sentences.
# It would be constant time complexity, the reason why is because you would go through each 10 pair of pants searching
# you find that which paints it is in and we dont know the number of which pants its in, it could be 6, 7 or even the last
# pair of pants. 
...
class PS5:
    def __init__(self,model, color, name,):
        ...
        self.name = name 
        def PS5Storage():
            print('PS5 has 1TB of storage ')
        def PS5Color():
            print('PS5 is Primary white and black')
        def PS5Power():
            if turnedon:
